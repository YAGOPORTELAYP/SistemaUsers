...
app.get('/users', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM usuarios') // Correção aqui
    res.json(rows)
  } catch (error) {
    res
      .status(500)
...
app.post('/users', async (req, res) => {
  ...
  try {
    const consulta =
      'INSERT INTO usuarios (nome_usuario, email, senha) VALUES ($1, $2, $3)' // Correção aqui
...
app.put('/users/:id', async (req, res) => {
  ...
  try {
    const consulta =
      'UPDATE usuarios SET nome_usuario = $1, email = $2, senha = $3 WHERE id = $4' // Correção aqui
...
app.delete('/users/:id', async (req, res) => {
  ...
  try {
    const consulta = 'DELETE FROM usuarios WHERE id = $1' // Correção aqui
